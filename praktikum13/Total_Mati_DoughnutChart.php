...
while($row = mysqli_fetch_array($covid)){
	...
	$query = mysqli_query($koneksi,"select sum(total_mati) as total_mati from covid where id_negara='".$row['id_negara']."'");
	$row = $query->fetch_array();
	$jumlah_covid[] = $row['total_mati'];
}
?>
<!doctype html>
<html>
...
<body>
	<h1>Doughnut Chart Total Kasus Meninggal</h1>
	...
	<script>
		var config = {
			...
			data: {
				datasets: [{
					label: 'Grafik Total kasus Meninggal',
